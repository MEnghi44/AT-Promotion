*** Settings ***
Documentation     Keyword สำหรับ flow ทดสอบโปรโมชัน "Free Item" บนหน้าจอ POS
...               (ซื้อ trigger_value ชิ้น ฟรี 1 ชิ้นตายตัว) ลอจิกหลักเหมือน
...               New Price ทุกอย่าง (สแกน/Add/สมาชิก/ยืนยัน) ต่างกันแค่สูตร
...               เช็คราคา - อยู่ใน FreeItemRobot.py ซึ่ง subclass มาจาก
...
...               new_price_keywords.robot - ถ้า calibrate พิกัดใหม่ที่นั่น
...               ต้องมาอัปเดตที่นี่ด้วย (เป็นเครื่อง POS จอเดียวกัน)
Library           ../libraries/CoordinateRobot.py    ../data/login_credentials.json
Library           ../libraries/FreeItemRobot.py    ../data/login_credentials.json
...
*** Keywords ***
# ใช้ชื่อเต็ม FreeItemRobot.xxx (ไม่ใช่แค่ "Set New Price Coordinate" เฉยๆ)
# เพราะ FreeItemRobot สืบทอด method ชื่อเดียวกันมาจาก NewPriceRobot - ถ้า
# import ทั้ง new_price_keywords.robot และ free_item_keywords.robot พร้อม
# กันในไฟล์ทดสอบเดียว (เช่น promotion.robot ที่เลือกรัน New Price หรือ
# Free Item ด้วย Run Keyword If) เรียกแบบไม่ระบุชื่อเต็มจะเจอ "Multiple
# keywords found" ทันที
Configure Free Item Coordinates
    FreeItemRobot.Set New Price Coordinate    barcode_input    ${BARCODE_INPUT_X}    ${BARCODE_INPUT_Y}
    FreeItemRobot.Set New Price Coordinate    member_button    ${MEMBER_BUTTON_X}    ${MEMBER_BUTTON_Y}
    FreeItemRobot.Set New Price Coordinate    member_barcode_input    ${MEMBER_BARCODE_INPUT_X}    ${MEMBER_BARCODE_INPUT_Y}
    FreeItemRobot.Set New Price Coordinate    member_welcome_confirm_button    ${MEMBER_WELCOME_CONFIRM_X}    ${MEMBER_WELCOME_CONFIRM_Y}
    FreeItemRobot.Set New Price Coordinate    member_welcome_header    ${MEMBER_WELCOME_HEADER_X}    ${MEMBER_WELCOME_HEADER_Y}
    FreeItemRobot.Set New Price Coordinate    receive_exact_button    ${RECEIVE_EXACT_BUTTON_X}    ${RECEIVE_EXACT_BUTTON_Y}
    FreeItemRobot.Set New Price Coordinate    popup_confirm_yes_button    ${POPUP_CONFIRM_YES_X}    ${POPUP_CONFIRM_YES_Y}
    FreeItemRobot.Set New Price Coordinate    sign_off_button    ${SIGN_OFF_BUTTON_X}    ${SIGN_OFF_BUTTON_Y}
    FreeItemRobot.Set New Price Coordinate    invalid_barcode_popup_point    ${INVALID_BARCODE_POPUP_X}    ${INVALID_BARCODE_POPUP_Y}
    FreeItemRobot.Set New Price Coordinate    mstamp_popup_check_point    ${MSTAMP_POPUP_CHECK_X}    ${MSTAMP_POPUP_CHECK_Y}
    FreeItemRobot.Set New Price Coordinate    mstamp_button    ${MSTAMP_BUTTON_X}    ${MSTAMP_BUTTON_Y}
    FreeItemRobot.Set New Price Price Point    ${PRICE_DISPLAY_X}    ${PRICE_DISPLAY_Y}
    FreeItemRobot.Set New Price Setting    step_delay_seconds    ${STEP_DELAY_SECONDS}
    FreeItemRobot.Set New Price Setting    db_price_check_wait_seconds    ${DB_PRICE_CHECK_WAIT_SECONDS}
    FreeItemRobot.Set New Price Setting    db_price_check_timeout_seconds    ${DB_PRICE_CHECK_TIMEOUT_SECONDS}
    FreeItemRobot.Set New Price Setting    lpe_check_timeout_seconds    ${LPE_CHECK_TIMEOUT_SECONDS}
    FreeItemRobot.Set New Price Setting    popup_timeout_seconds    ${POPUP_TIMEOUT_SECONDS}
    FreeItemRobot.Set New Price Setting    receive_exact_retry_count    ${RECEIVE_EXACT_RETRY_COUNT}

Run Free Item Promotion Test
    Configure Free Item Coordinates
    ${log_path}=    FreeItemRobot.Run New Price Test Suite
    FreeItemRobot.Sign Off
    Should Not Be Empty    ${log_path}
    Log    บันทึกผลการทดสอบ Free Item ที่: ${log_path}
